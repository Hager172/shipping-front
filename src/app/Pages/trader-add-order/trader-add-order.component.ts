...
@Component({
  ...
  templateUrl: './trader-add-order.component.html',
  styleUrls: ['./trader-add-order.component.css']
})
export class TraderAddOrderComponent implements OnInit {
  ...
  ngOnInit(): void {
    const storedTraderId = localStorage.getItem('traderId');
    if (storedTraderId) {
      this.order.traderId = +storedTraderId;
    }
